        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    outputs = torch.zeros(N, self.batch_size, 10)
                    log_priors = torch.zeros(N)
                    log_variational_posteriors = torch.zeros(N)
                    for i in range(0,N):
                        outputs[i], log_priors[i], log_variational_posteriors[i]=self.network(batch_x)
                    log_prior = log_priors.mean()
                    log_variational_posterior = log_variational_posteriors.mean()
                    negative_log_likelihood = F.nll_loss(F.log_softmax(outputs.mean(0), dim=1), batch_y, reduction='sum')
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.weights_var_posterior = MultivariateDiagonalGaussian(
                mu=nn.Parameter(torch.Tensor(in_features * out_features).uniform_(-0.2, 0.2)),
                rho=nn.Parameter(torch.Tensor(in_features * out_features).uniform_(-5.0, -4.0)))
        ...
        if self.use_bias:
            ...
            self.bias_var_posterior = MultivariateDiagonalGaussian(
                    mu=nn.Parameter(torch.Tensor(out_features).uniform_(-0.2, 0.2)),
                    rho=nn.Parameter(torch.Tensor(out_features).uniform_(-5.0, -4.0)))
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        weights = torch.reshape(weights, (self.out_features, self.in_features))
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for idx, current_layer in enumerate(self.layers):
            new_features, layer_log_prior, layer_log_variational_posterior = current_layer(output_features)
            if idx < len(self.layers) - 1:
                new_features = self.activation(new_features)

            log_prior += layer_log_prior
            log_variational_posterior += layer_log_variational_posterior
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        return distribution.log_prob(values).sum()
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        distribution = Normal(self.mu, sigma)
        return distribution.log_prob(values).sum()

    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        sigma = F.softplus(self.rho)
        distribution = Normal(torch.zeros_like(self.mu), torch.ones_like(sigma))
        x = self.mu + distribution.sample() * sigma
        return x
