...
class BO_algo(object):
    def __init__(self):
        ...
        self.constraint_model = GaussianProcessRegressor(
                kernel = ConstantKernel(constant_value = 3.5) \
                         * RBF(length_scale = 2),
                alpha = 0.005**2)  # TODO : GP model for the constraint function
        self.objective_model = GaussianProcessRegressor(
                kernel = ConstantKernel(constant_value = 1.5) \
                        * RBF(length_scale = 1.5),
                alpha = 0.01**2)  # TODO : GP model for your objective function
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        # TODO: enter your code here
        mean, std = self.objective_model.predict(x.reshape(1, -1), return_std = True)

        # One possibility: GP Upper Confidence Bound
        # k = 2 # parameter of GPUCB
        # a = mean - k*std

        # Another possibility: Expected Improvement
        best_f = (np.array(self.previous_points)[:,2]).min() if len(self.previous_points) else mean
        gamma = (best_f - mean)/std
        a = std * (gamma * norm.cdf(gamma) + norm.pdf(gamma))

        # Include the constraint
        con_mean, con_std = self.constraint_model.predict(x.reshape(1, -1), return_std=True)
        a *= norm.cdf((LAMBDA - con_mean) / con_std)
    ...
    def get_solution(self) -> np.ndarray:
        ...
        # Among queried points that satisfy the constraint, return the one with minimum objective value.
        prev_pts = np.array(self.previous_points)
        valid_pts = prev_pts[prev_pts[:,3] <= LAMBDA]  # all points satisfying the constraint
        if valid_pts.size == 0:  # if there are no valid points, return a random point
            print (":(")
            return np.random.uniform(0, 6, 2)
        opt_ind = valid_pts[:,2].argmin()  # index of point with minimum objective value
        x = valid_pts[opt_ind,:2]
        return x

#        SAMPLES = 4000
#        pts = np.random.uniform(0, 6, (SAMPLES, 2))
#        constraints, std = self.constraint_model.predict(pts, return_std=True)
#        valid_pts = pts[constraints + 2 * std <= LAMBDA]
#        if valid_pts.size == 0:
#            print(":(")
#            valid_pts = pts[constraints.argmin()].reshape(1, -1)
#        objectives = self.objective_model.predict(valid_pts)
#        return valid_pts[objectives.argmin()]
