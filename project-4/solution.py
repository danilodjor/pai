...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            action_pd,_=self.pi(state)
            act=action_pd.sample()
            action_prob=self.pi._log_prob_from_distribution(action_pd, act)
            value_fcn=self.v(state)
            return act.item(),value_fcn.item(),action_prob.item()
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        vals_temp=vals.copy()
        vals_temp=np.insert(vals_temp,0,0)
        deltas = rews+self.gamma*(vals_temp[:-1])-vals
        for i in range(0,len(deltas)-1):
            self.phi_buf[self.path_start_idx+i]=discount_cumsum(deltas[i:len(deltas)],self.gamma*self.lam)[0]
        ...
        for i in range(0,len(rews)-1):
            self.ret_buf[self.path_start_idx+i]=discount_cumsum(rews[i:len(rews)],self.gamma)[0]
...
class Agent:
    ...
    def pi_update(self, data):
        ...
        for i in range(0,len(obs)):
            _,log_prob=self.ac.pi(obs[i],act[i])
            loss.append(-phi[i]*log_prob)
        sum(loss).backward()
    ...
    def v_update(self, data):
        ...
        loss=Functional.mse_loss(ret,vals)
        loss.backward()
        print("Loss on value fcn estimation:"+str(loss.item()))
    ...
    def get_action(self, obs):
        ...
        act_dist,_=self.ac.pi(torch.as_tensor(obs, dtype=torch.float32))
        act=act_dist.sample()
        return act.item()
